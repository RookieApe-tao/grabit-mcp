...
function evalRational(s, fallback) {
    if (!s)
        return fallback;
    const m = s.match(/^(-?[\d.]+)\/(-?[\d.]+)$/);
    if (m) {
...
async function probeVideo(ffprobe, file) {
    const { stdout } = await runp(ffprobe, [
        "-v", "error",
        "-select_streams", "v:0",
    ...
    if (!vs)
        throw new Error("探测失败：文件里没有视频流");
    const hasAudio = await runp(ffprobe, ["-v", "error", "-select_streams", "a:0", "-show_entries", "stream=index", "-of", "json", file], { timeout: 30_000, maxBuffer: 8 * 1024 * 1024 })
...
function buildDropPlan(duration, fps, window, min, max, rng) {
    const times = [];
    let windows = 0;
    for (let wStart = 0; wStart + 0.2 < duration && times.length < MAX_DROP_TERMS; wStart += window) {
        ...
        for (let i = 0; i < want; i++)
            picked.add(firstFrame + Math.floor(rng() * span));
...
function dropExpressionVideo(times, fps) {
    const eps = 0.3 / fps;
    return times
...
function dropExpressionAudio(times, fps) {
    const slot = 1 / fps;
    return times
...
export async function mediaRemix(file, o = {}) {
    ...
    if (o.speed === "off")
        speed = 1;
    else if (o.speed === "auto" || o.speed === undefined)
        speed = Math.round((0.97 + rng() * 0.09) * 1000) / 1000;
    else
        speed = Math.min(4, Math.max(0.25, o.speed));
    ...
    let targetFps = null;
    if (o.fps === "auto" || o.fps === undefined)
        targetFps = probe.fps > 30.5 ? 30 : null;
    else if (o.fps !== "off")
    ...
    if (hasSelect)
        vf.push(`select='not(${dropExpressionVideo(plan.times, probe.fps)})'`);
    if (needSetpts)
        vf.push(`setpts=N/${outRate.toFixed(6)}/TB`);
    if (targetFps && Math.abs(targetFps - outRate) > 0.5)
        vf.push(`fps=${targetFps}`);
    else if (needSetpts)
        vf.push(`fps=${outRate.toFixed(6)}`);
    if (noise > 0)
        vf.push(`noise=alls=${noise}:allf=t+u`);
    // 滤镜链（音频）：同一批时间点删片段（保持音画同步）→ 变速
    const af = [];
    if (probe.hasAudio) {
        if (hasSelect)
            af.push(`aselect='not(${dropExpressionAudio(plan.times, probe.fps)})'`, "asetpts=N/SR/TB");
        if (speed !== 1)
    ...
    if (vf.length)
        baseArgs.push("-vf", vf.join(","));
    if (af.length)
        baseArgs.push("-af", af.join(","));
    baseArgs.push("-c:v", "libx264", "-crf", "17", "-preset", "fast", ...(probe.hasAudio ? ["-c:a", "aac", "-b:a", "192k"] : ["-an"]), "-movflags", "+faststart");
    const runFfmpeg = (extra) => new Promise((resolve, reject) => {
    ...
    try {
        ...
        if (/fps_mode|unrecognized option|unknown option/i.test(msg))
            await runFfmpeg(["-vsync", "0"]);
        else
            throw e;
    }
    if (!fs.existsSync(out))
        throw new Error("混剪输出未生成");
    // 诚实上报：无 fps 重采样时按「源帧数 - 成品帧数」实测删帧数（select 按 t 匹配可能偶发脱靶）
    let droppedFrames = plan.times.length;
    if (!targetFps && probe.nbFrames > 0) {
        const outFrames = await runp(ffprobe, ["-v", "error", "-select_streams", "v:0", "-show_entries", "stream=nb_frames", "-of", "csv=p=0", out], { timeout: 30_000, maxBuffer: 8 * 1024 * 1024 })
            .then(({ stdout }) => parseInt(stdout.trim(), 10) || 0)
            .catch(() => 0);
        if (outFrames > 0)
            droppedFrames = Math.max(0, probe.nbFrames - outFrames);
    }
...
/** 目录批量时跳过已是混剪成品的文件 */
export function isRemixOutput(name) {
