...
async function ensureOcrData(onLine) {
...
async function grabFrame(ffmpeg, file, t, out) {
    try {
        await runp(ffmpeg, ["-y", "-v", "error", "-ss", t.toFixed(3), "-i", file, "-frames:v", "1", "-an", "-vf", "scale='min(1280,iw)':-2", out], { timeout: 60_000, maxBuffer: 4 * 1024 * 1024 });
 ...
 */
export async function detectOnScreenText(ffmpeg, file, duration, o = {}) {
    ...
    for (let i = 0; i < maxFrames; i++) {
        ...
        if (!times.some((x) => Math.abs(x - t) < 0.2))
            times.push(t);
    }
    ...
    try {
        ...
        for (const t of times) {
            ...
            if (!(await grabFrame(ffmpeg, file, t, png)))
                continue;
            framesChecked++;
            let text = "";
            try {
            ...
            if (found)
                return { found: true, framesChecked, sample };
        }
