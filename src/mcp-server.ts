...
server.tool(
  "media_download",
  "下载视频/音频到本机（默认最高画质、ffmpeg 合并）。视频默认下载后自动「混剪+画质优化」并删除原片只留成品（智能镜像：检测到画面文字自动改用裁剪；另有抽帧/变速/噪点/每10秒随机删帧），remix=false 可关闭",
  {
    url: z.string().describe("视频页面链接"),
    quality: z
...
server.tool(
  "media_remix",
  "本地混剪去重：对已有视频做智能镜像（OCR 检测到画面文字自动改随机裁剪放大）/抽帧/变速/加噪点/每10秒随机删1~3帧，并顺带轻画质优化，一次编码输出 _混剪优化.mp4（默认保留源文件）",
  {
    file: z.string().describe("视频文件完整路径"),
  ...
  async ({ file, mirror, textDetect, zoomMin, zoomMax, speed, noise, fps, dropWindow, dropMin, dropMax, seed, enhance, deleteSource }) => {
    try {
      const o: RemixOptions = { mirror, textDetect, zoomMin, zoomMax, speed, noise, fps, dropWindow, dropMin, dropMax, seed, enhance, deleteSource };
