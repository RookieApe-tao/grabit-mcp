...
type Parsed = { get: (...names: string[]) => string | undefined; flag: (name: string) => boolean };
...
async function main(): Promise<number> {
  ...
  switch (cmd) {
    ...
    case "batch": {
      ...
      const results = await mediaBatch(
        lines,
        {
    ...
    case "remix": {
      ...
      let speed: number | "auto" | "off" = "auto";
      if (speedRaw === "off") speed = "off";
      ...
      let fps: number | "auto" | "off" = "auto";
      if (fpsRaw === "off") fps = "off";
      ...
      const numOf = (name: string, def: number, lo: number, hi: number): number => {
        const raw = get(name);
    ...
    case "config": {
      ...
      if (value === undefined) {
        console.log(`${key} = ${(cfg as unknown as Record<string, unknown>)[key] ?? "(未设置)"}`);
        return 0;
      }
      const next = { ...cfg } as unknown as Record<string, unknown>;
      if (value === "" || value === "(clear)") delete next[key];
      else next[key] = value;
      saveConfig(next as unknown as GrabitConfig);
