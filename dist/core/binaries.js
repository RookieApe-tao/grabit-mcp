...
function localBin(cfg, name) {
    return path.join(cfg.binDir, process.platform === "win32" ? `${name}.exe` : name);
}
export async function resolveYtDlp(cfg) {
    if (cfg.ytDlpPath && fs.existsSync(cfg.ytDlpPath))
...
export async function resolveFfmpeg(cfg) {
...
export async function doctor(cfg) {
    ...
    if (!ytDlp)
        report.hints.push("缺少 yt-dlp：运行 `grabit doctor --fix` 自动下载，或 winget install yt-dlp.yt-dlp");
    if (!ffmpeg)
        report.hints.push("缺少 ffmpeg：运行 `grabit doctor --fix` 自动下载，或 winget install Gyan.FFmpeg");
    return report;
}
async function downloadTo(url, dest, attempts = 3) {
    ...
    for (let i = 1; i <= attempts; i++) {
        try {
            ...
            if (buf.length < 1024)
                throw new Error(`文件过小 (${buf.length}B)，疑似下载失败`);
...
export async function installBinaries(cfg) {
    ...
    if (!ytDlp) {
        ...
        await downloadTo("https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe", localBin(cfg, "yt-dlp"));
    ...
    if (!ffmpeg) {
        ...
        try {
            ...
            await runp("powershell", ["-NoProfile", "-Command", `Expand-Archive -LiteralPath '${zip}' -DestinationPath '${exDir}' -Force`], { timeout: 300_000 });
        }
